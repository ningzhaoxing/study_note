...
`sessions_spawn` 产生的"子 Agent"有两种本质上不同的形态，不可混淆：

#### 形态一：Subagent（`runtime="subagent"`）

**当前 Pi Agent 的一个隔离副本**，使用 `minimal` System Prompt 模式（只保留 Tooling、Workspace、Runtime 三个 section，去掉 Skills、Memory、Messaging 等，大幅减少 token 开销）。用于将可并行或后台执行的子任务分离出去：

```text
主 Agent 收到："帮我同时审查这三个 PR"
    ↓
sessions_spawn(task="审查 PR #86", runtime="subagent")  ─┐
sessions_spawn(task="审查 PR #87", runtime="subagent")   ├─ 三个副本并行执行
sessions_spawn(task="审查 PR #88", runtime="subagent")  ─┘
    ↓
各子 Agent 完成后推送通知给父 Agent → 父 Agent 汇总回复
```

本质上是**同一套代码**，只是在独立 session 里运行，专注处理委派的子任务。

#### 形态二：ACP 编码 Agent（`runtime="acp"`）

完全不同的东西——通过 ACP（Agent Coding Protocol）启动**外部独立编码 Agent 进程**（Codex CLI、Claude Code、OpenCode、Pi coding agent 等）：

```text
主 Agent 收到："用 Claude Code 帮我重构这个模块"
    ↓
sessions_spawn(task="重构 auth 模块", runtime="acp", agentId="claude-code")
    ↓
实际执行：claude --permission-mode bypassPermissions --print "重构 auth 模块"
    ↓
Claude Code 进程跑完 → 结果通过 ACP 协议回传
```

ACP 子 Agent 不是 Pi Agent 的副本，是完全独立的外部可执行程序。
...
### 11.0 重要澄清：OpenClaw 是单 Agent 框架

**OpenClaw 只有一种 Agent 类型——Pi Agent**，它并不存在"资产识别 Agent""威胁分析 Agent"等专门化 Agent。所谓"多 Agent 路由"，是指**同一种 Pi Agent 的多个不同配置实例**之间的路由，本质是"多个人格/工作空间"，而非"多种专业能力"。

```text
Gateway
├── agent "work"     → Pi Agent（workspace=~/work，skill集合A，model=claude-opus）
├── agent "personal" → Pi Agent（workspace=~/personal，skill集合B，model=gpt-4o）
└── agent "coding"   → Pi Agent（workspace=~/code，skill集合C，thinking=high）
```

**专业能力的差异完全由 Skill 承载，而不是由不同的 Agent 类型承载。** 这正是 Skill 系统的核心价值：一个通用 Agent + 可插拔的领域 Skill，替代多个专门化 Agent。

### 11.1 渠道路由

在 `openclaw.json` 的 `agents` 配置中，可以将不同渠道/账户/联系人路由到不同的 Agent 配置实例：
...
路由的维度是**渠道/用户身份 → Agent 配置**，不是**任务类型 → 专门化 Agent**。每个配置实例有独立的 Workspace 和会话隔离，但底层跑的是完全相同的 Pi Agent 代码。

### 11.2 会话隔离维度

| 维度 | 隔离方式 |
| --- | --- |
| 渠道 | 按 channel 类型路由到不同 Agent 配置 |
